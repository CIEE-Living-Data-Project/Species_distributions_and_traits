...
convertRateInd2WS <- function(df, sizeAssoc, trtName1, trtName2, trtUnit) {
  ...
  D.2 <- df %>% 
    ...
    mutate(verbatimScientificName = scientificName,
           ...
           isMerged = FALSE,
           isDerived = TRUE,
           traitTaxonIDSource = traitTaxonID) %>% 
    ...
    mutate(traitName = trtName1,
           ...
           traitUnit = trtUnit,
           traitValueSource = "derived from related trait",
           traitValueSD = NaN,
           traitValueN = 1,
...
mergeTraitDetails <- function(df, trait.directory, rev.by = "P. Pata") {
  ...
  df.updated <- df %>% 
    ...
    mutate(primaryReference = paste(primaryReference, collapse = "; "),
           ...
           traitTaxonIDSource = paste(traitTaxonID, collapse = "; ") ) %>% 
    # clean strings
    mutate(primaryReference = cleanStrings(primaryReference),
           ...
           traitTaxonIDSource = cleanStrings(traitTaxonID) )%>% 
    distinct(traitTaxonIDSource, .keep_all = TRUE) %>% 
    ungroup() %>% 
...
#   traitTaxonIDSource is not empty, i.e., the record was based on something else.
removeVerbatimRecords <- function(df) {
  ...
  df.A <- df %>% 
    filter(is.na(traitTaxonIDSource))
...
updateIDs <- function(data) {
  ...
  load("data_raw/master_id_list.RData")
  ...
  save(master.ID.List, file = "data_raw/master_id_list.RData")
...
standardizeTable <- function(file.list, s.format, taxonomy, lifestagelist,
  ...
  data <- s.format
    ...
    # dplyr::select(-c(traitTaxonIDSource, maxObsNum, isMerged))
  ...
  for(i in file.list) {
    ...
    if("traitValueTemperature" %in% colnames(d)) {
      ...
      d$verbatimTemperature <- as.character(d$traitValueTemperature)
    }
  ...
  data <- data %>% 
    distinct(verbatimScientificName, verbatimTraitName, verbatimTraitUnit, 
             ...
             traitValueTemperature, .keep_all = TRUE)
...
adjustClass <- function(data, s.format){
  data <- data %>% 
    mutate(traitUnit = as.character(traitUnit),
           ...
           traitValueMeasurementNotes = as.character(traitValueMeasurementNotes),
           ...
           traitTaxonIDSource = as.character(traitTaxonIDSource),
           isMerged = as.logical(isMerged),
...
#   generate an updated traitTaxonID and this should created if including the 
...
getGroupLevelValue <- function(taxon, trait, gen.level = "genus", trait.df, taxonomy.df){
  ...
  generalized.trait <- trait.sub %>% 
    ...
    mutate(traitValueSD = sd(traitValue, na.rm = TRUE), traitValueN = n(),
...
calculateFromModel <- function(df, model, trait.directory, excludeWithLit = TRUE,
  ...
  df.withdata <- df %>% 
    filter(str_detect(traitValueSource, "literature|calculated taxon average")) %>% 
  ...
  df.calc <- df %>% 
    ...
    select(-c(primaryReferenceDOI, secondaryReferenceDOI, traitTaxonID, 
              ...
              sizeAssocReference, location, decimalLongitude, decimalLatitude, notes,
              isMerged, isDerived, traitTaxonIDSource, traitValueMeasurementNotes)) %>%  
  ...
  if(excludeCalculated == TRUE) {
    df.calc <- df.calc %>% 
      filter(!(traitValueSource == "calculated from regression"))
  }
  df.calc <- df.calc %>% 
...
widen_traits_SE <- function(trait.table, trait.list) {
  ...
  sd <- trait.table %>%
    filter(traitName %in% trait.list) %>%
    dplyr::select(taxonID, scientificName, majorgroup, aphiaID,
                  traitName, traitValueSD) %>%
    pivot_wider(names_from = traitName, values_from = traitValueSD) %>% 
    relocate(all_of(trait.list)) %>% 
  ...
  n <- trait.table %>%
    filter(traitName %in% trait.list) %>%
    dplyr::select(taxonID, scientificName, majorgroup, aphiaID,
                  traitName, traitValueN) %>%
    pivot_wider(names_from = traitName, values_from = traitValueN) %>% 
    relocate(all_of(trait.list)) %>% 
